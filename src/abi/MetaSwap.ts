...
export interface TokenSwapUnderlying0Event {
  buyer: string;
  tokensSold: ethers.BigNumber;
  tokensBought: ethers.BigNumber;
  soldId: ethers.BigNumber;
...
export const events = {
  ...
  "TokenSwapUnderlying(address,uint256,uint256,uint128,uint128)":  {
    ...
    decode(data: EvmEvent): TokenSwapUnderlying0Event {
      const result = abi.decodeEventLog(
        abi.getEvent("TokenSwapUnderlying(address,uint256,uint256,uint128,uint128)"),
        data.data || "",
        data.topics
      );
      return  {
